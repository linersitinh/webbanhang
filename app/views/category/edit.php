...
<section class="w-full max-w-4xl mx-auto px-4 md:px-margin-desktop py-12">
    ...
    <h1 class="font-display-lg-mobile text-display-lg-mobile text-on-surface">Sửa Danh Mục</h1>
</section>
...
<section class="w-full max-w-4xl mx-auto px-4 md:px-margin-desktop pb-32">
    <div class="bg-surface border border-outline-variant/20 rounded-lg p-8">
        <form method="POST" action="/webbanhang/Category/update" class="space-y-6">
            <input type="hidden" name="id" value="<?php echo $category->id; ?>">
            
            <!-- Name Field -->
            <div>
                <label for="name" class="block font-label-md text-label-md text-on-surface mb-2">Tên Danh Mục *</label>
                <input type="text" id="name" name="name" class="w-full px-4 py-3 border border-outline-variant/30 rounded bg-surface focus:outline-none focus:border-tertiary transition-colors font-body-md text-body-md" placeholder="Nhập tên danh mục" value="<?php echo htmlspecialchars($category->name); ?>" required>
            </div>
            
            <!-- Description Field -->
            <div>
                <label for="description" class="block font-label-md text-label-md text-on-surface mb-2">Mô Tả</label>
                <textarea id="description" name="description" rows="4" class="w-full px-4 py-3 border border-outline-variant/30 rounded bg-surface focus:outline-none focus:border-tertiary transition-colors font-body-md text-body-md" placeholder="Nhập mô tả danh mục"><?php echo htmlspecialchars($category->description); ?></textarea>
            </div>
